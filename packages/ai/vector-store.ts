...
export class VectorStoreService {
  ...
  startIngestion(
    ...
    if (this.isIngesting || this.ingestionComplete) {
      logger.warn("Ingestion already started or completed");
      return;
    }

    this.configureModel();
    logger.debug("Step 1: Embedding model configured successfully");
  ...
  private async performIngestion(
    ...
    try {
      logger.debug(`Processing ${mountData.length} mount paths...`);
      ...
      for (const { hostPath, targetPath, files } of mountData) {
        ...
        for (const { path, content } of files) {
          ...
          if (this.shouldSkipFile(path)) {
            skippedFiles++;
            logger.debug(`Skipping file: ${path} (filtered)`);
            continue;
          }
      ...
      if (ingestedFiles > 0) {
        ...
        try {
          ...
          logger.debug("Loading documents with SimpleDirectoryReader...");
          ...
          if (documents.length > 0 && tempDir) {
            ...
            logger.debug("Document metadata paths fixed");
          }
          ...
          if (documents.length > 0) {
            ...
            this.index = await VectorStoreIndex.fromDocuments(documents, {
              logProgress: logger.getLevel() === LogLevel.DEBUG,
            });
            logger.debug("VectorStoreIndex created successfully");
            ...
            logger.warn("No documents loaded by SimpleDirectoryReader");
          }
        } catch (indexError) {
      ...
      logger.error("Error in performIngestion", {
        error: String(error),
        stack: error instanceof Error ? error.stack : undefined,
      });
      throw error;
  ...
  async query(queryText: string): Promise<string> {
    ...
    try {
      ...
      logger.error("Query execution failed", { error: String(error) });
      throw new Error(
        `Query failed: ${
  ...
  async retrieveFilePaths(queryText: string): Promise<string> {
    ...
    logger.info(`Executing file path retrieval for query: "${queryText}"`);
    ...
    try {
      ...
      logger.error("File path retrieval execution failed", {
        error: String(error),
      });
      throw new Error(
  ...
  getAllIndexedFilePaths(): string {
    ...
    logger.info("Retrieving all indexed file paths from stored list");
...
export function getVectorStore(): VectorStoreService {
  if (!vectorStoreInstance) {
    try {
      ...
      logger.debug("VectorStoreService singleton created");
    } catch (error) {
      logger.error("Failed to create VectorStoreService singleton", {
        error: String(error),
      });
      throw error;
    }
