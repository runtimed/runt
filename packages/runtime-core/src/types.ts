...
/**
 * Platform-agnostic configuration options for runtime agents
 *
 * The runtime agent now takes a Store as a constructor parameter,
 * removing the need for platform-specific configuration.
 */
export interface RuntimeAgentOptions {
  ...
  runtimeType: string;
  /** Client ID - must be authenticated user ID for consistency */
  clientId: string;
