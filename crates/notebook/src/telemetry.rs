//! Anonymous telemetry collection using Ed25519 signatures.
//!
//! # Security Model
//!
//! Each installation generates a unique Ed25519 keypair on first use:
//! - Private key stays local, used to sign telemetry submissions
...
const TELEMETRY_URL: &str = "https://telemetry.runt.dev/telemetry";
...
impl TelemetryEvent {
    /// Create a new telemetry event with the current timestamp.
...
/// Payload sent to the telemetry server.
...
/// Path to the telemetry key file.
fn telemetry_key_path() -> Option<PathBuf> {
    // Allow override for testing
    if let Ok(path) = std::env::var("RUNT_TELEMETRY_KEY_PATH") {
        return Some(PathBuf::from(path));
    }
    dirs::config_dir().map(|d| d.join("runt").join("telemetry-key"))
}

/// Get or create the telemetry signing key.
///
/// The key is stored as a 32-byte seed in the config directory.
/// Returns None if key generation/loading fails (telemetry will be disabled).
fn get_or_create_signing_key() -> Option<SigningKey> {
    let key_path = telemetry_key_path()?;

    if key_path.exists() {
        // Read existing key
        let key_bytes = std::fs::read(&key_path).ok()?;
        if key_bytes.len() != 32 {
            log::warn!("Telemetry key file is corrupted, regenerating");
...
fn create_new_key(key_path: &PathBuf) -> Option<SigningKey> {
    ...
    log::info!("Generated new telemetry key");
    Some(signing_key)
}
...
pub fn send_events(events: Vec<TelemetryEvent>) {
    ...
    let Some(key) = signing_key() else {
        log::debug!("Telemetry disabled (no signing key)");
        return;
    };
    ...
    let events_json = match serde_json::to_string(&events) {
        Ok(json) => json,
        Err(e) => {
            log::warn!("Failed to serialize telemetry events: {}", e);
            return;
        }
    };
...
pub fn send_event(event: TelemetryEvent) {
    send_events(vec![event]);
}

/// Actually send the payload to the server.
async fn send_payload(payload: TelemetryPayload) {
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            log::debug!("Failed to create HTTP client for telemetry: {}", e);
            return;
        }
    };
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            log::debug!("Failed to send telemetry (server may be offline): {}", e);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_telemetry_event_with_data() {
        let event = TelemetryEvent::new("test_event")
    ...
    fn test_key_generation_and_loading() {
        ...
        let key_path = temp_dir.path().join("telemetry-key");

        std::env::set_var("RUNT_TELEMETRY_KEY_PATH", key_path.to_str().unwrap());
        ...
        std::env::remove_var("RUNT_TELEMETRY_KEY_PATH");
    }
